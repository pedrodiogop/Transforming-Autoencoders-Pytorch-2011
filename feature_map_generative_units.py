...
def plot_feature_maps_rows(caps_imgs, inp, out, target, all_probs, RESULTS_DIR_IN_OUT_TARGET_IMAGES, transformations, label, cmap='viridis'):
    rows, indices = build_capsule_sum_pyramid_by_max(caps_imgs)
    all_probs = all_probs.detach().cpu().flatten()
    probs = all_probs[indices[0]]
    n_caps = caps_imgs.shape[0]
    nrows = len(rows)
    ncols_max = n_caps  # o nível 0 (cápsulas originais) define a largura do grid
    ...
    fig, axes = plt.subplots(nrows, ncols_max, figsize=(ncols_max * 2, nrows * 2))
    ...
    for r, row_tensor in enumerate(rows):
        imgs = row_tensor.detach().cpu().numpy()
        if imgs.ndim == 4:
            imgs = imgs.squeeze(1)

        positions = col_positions[r]
        ...
        for i, col in enumerate(positions):
            ...
            im = ax.imshow(img_clamped, cmap='gray', vmin=0.0, vmax=1.0)
            ...
            if r == 0:
               ...
               # ax.set_title(f'[{vmin:.2f}, {vmax:.2f}]', fontsize=8)
    ...
    file_path = f'{RESULTS_DIR_IN_OUT_TARGET_IMAGES}/Capsules/{label}/{transformations_str}'
    ...
    # imagem final da pirâmide de somas (soma de todas as cápsulas)
    final_sum_image = rows[-1].detach().cpu()   # shape [1, C, H, W]
    final_sum_image = final_sum_image.squeeze(0)  # shape [C, H, W]
    final_sum_image = torch.sigmoid(final_sum_image).detach().cpu() if ('CIFAR' not in DATASET and 'Mine' not in DATASET) else out.clamp(0, 1).detach().cpu()
...
def PlotGenerativeFeatureMaps(capL, X, delxy, img_h, img_w, RESULTS_DIR_FEATURES, num_capsule, num_generative):

    os.makedirs(RESULTS_DIR_FEATURES, exist_ok=True)
    ...
    fig.suptitle(
        f'Feature maps das unidades generativas',
        fontsize=6
    )
    ...
    with torch.no_grad():
        ...
        for k in range(num_capsule):
            ...
            _, _, _, gen = cap(X, delxy)


            # média das ativações do batch
            ...
            for p in range(num_generative):
                ...
                # vetor só com a unidade p ativa
    ...
    plt.savefig(
        f'{RESULTS_DIR_FEATURES}/feature_maps_epooch.png',
        dpi=150,
        bbox_inches='tight'
    )
...
if __name__ == '__main__':
    ...
    image_index = 6  # Index of the image to visualize
    ...
    with torch.no_grad():
        ...
        capsule_stacked, reconstruction_image, all_probs, gen = capL_feature_map(sample, transformations)
        ...
        # print(caps_imgs.size()) # torch.Size([25, 1, 28, 28])
        Save_Feature_Map_Images(sample, target, reconstruction_image, caps_imgs, label, RESULTS_DIR_FEATURE_MAP, DATASET, transformations)
        # plot_feature_maps_rows(caps_imgs, sample, reconstruction_image, target, all_probs, RESULTS_DIR_FEATURE_MAP, transformations, label)
        # PlotGenerativeFeatureMaps(capL_feature_map, sample, transformations, IMG_H, IMG_W, RESULTS_DIR_FEATURE_MAP, NUM_CAPS, CAP_GEN)
